...
// 节点类型图标/标签、关系类型配色统一取自 core/types.js
// （TYPE_ICONS / TYPE_LABELS / RELATION_TYPES / RELATION_COLORS）
...
function _buildFilters(){
  ...
  el.innerHTML = types.map(t=>{
    ...
    return `<button class="gchip${off?' off':''}" data-type="${t}" title="${off?'显示':'隐藏'}${TYPE_LABELS[t]||t}">
      <span class="gchip-ico">${TYPE_ICONS[t]||'●'}</span>${TYPE_LABELS[t]||t}</button>`;
  }).join('');
  el.querySelectorAll('.gchip').forEach(b=>{
    b.addEventListener('click', ()=>{
      const t=b.dataset.type, lbl=TYPE_LABELS[t]||t;
...
function _buildGraph(data){
  ...
  const _relGlobals = RELATION_COLORS;
  const relColors = Object.assign({}, _relGlobals);
  RELATION_TYPES.forEach(r=>{ if(!relColors[r.value]) relColors[r.value]=r.color; });
...
function _buildLegend(box, usedTypes){
  const relList = RELATION_TYPES;
  const relItems = relList.map(r=>`<span class="lg-item"><span class="lg-line" style="background:${r.color}"></span>${ESC(r.label)}</span>`).join('');
  const typeItems = usedTypes.map(t=>`<span class="lg-item"><span class="lg-ico">${TYPE_ICONS[t]||'●'}</span>${ESC(TYPE_LABELS[t]||t)}</span>`).join('');
